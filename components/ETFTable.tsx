...
function ReturnCell({ value }: { value: number | null }) {
  if (value === null || value === undefined) return <td className="px-3 py-2.5 text-center text-[#dddddd]">---</td>;
...
export default function ETFTable({ etfs, onSelect, selectedTickers, onToggleCompare }: Props) {
  ...
  const SortHeader = ({ label, field, align }: { label: string; field: SortKey; align?: string }) => (
    <th
      className={`px-3 py-3 text-[9px] font-mono font-bold text-[#999] uppercase tracking-[0.15em] cursor-pointer hover:text-[#3d52ef] select-none transition-colors ${align || "text-left"}`}
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex items-center gap-4">
        <div className="relative flex-1 max-w-lg">
          ...
          <input
            ...
            className="w-full pl-7 pr-4 py-2.5 bg-white border border-[--border-subtle] rounded-lg text-[#202020] font-mono text-sm placeholder-[#c6c6c6] focus:outline-none focus:border-[#3d52ef] focus:shadow-[0_0_0_1px_rgba(61,82,239,0.15)] transition-all"
          />
        </div>
        {selectedTickers.length > 0 && (
          <span className="text-[10px] font-mono text-[#3d52ef] bg-[#3d52ef]/5 px-3 py-1.5 rounded border border-[#3d52ef]/15">
      ...
      {/* Table */}
      <div className="rounded-lg border border-[--border-subtle] overflow-hidden bg-white shadow-sm glow-hover">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="bg-[--bg-raised] border-b border-[--border-subtle]">
                <th className="px-3 py-3 text-center text-[9px] font-mono text-[#999] w-10">
                  <span className="text-[#3d52ef]">#</span>
                </th>
                <SortHeader label="Ticker" field="ticker" />
                <th className="px-3 py-3 text-left text-[9px] font-mono font-bold text-[#999] uppercase tracking-[0.15em]">Nome</th>
                <th className="px-3 py-3 text-left text-[9px] font-mono font-bold text-[#999] uppercase tracking-[0.15em]">Bench</th>
            ...
            <tbody>
              {sorted.map((etf, idx) => (
                <tr
                  key={etf.ticker}
                  className={`border-b border-[--border-muted] cursor-pointer transition-all hover:bg-[#3d52ef]/[0.03] ${
                    selectedTickers.includes(etf.ticker) ? "bg-[#3d52ef]/[0.05]" : idx % 2 === 0 ? "bg-white" : "bg-[--bg-raised]"
                  }`}
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-[12px] text-[#515151] max-w-[180px] truncate" title={etf.nome}>
                    {etf.nome_curto || etf.nome}
                  </td>
                  <td className="px-3 py-2.5 text-[11px] font-mono text-[#999]">{etf.benchmark || "---"}</td>
                  <td className="px-3 py-2.5 text-[11px] text-[#999] max-w-[120px] truncate" title={etf.admin || ""}>
                    {etf.admin || "---"}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-[13px] text-[#202020]">
                    {etf.preco ? `${etf.preco.toFixed(2)}` : "---"}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-[12px] text-[#999]">
                    {etf.taxa_adm !== null ? `${etf.taxa_adm.toFixed(2)}%` : "---"}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-[12px] text-[#999]">
                    {formatNumber(etf.patrimonio)}
                  </td>
                  <td className="px-3 py-2.5 text-center font-mono text-[11px] text-[#c6c6c6]">
                    {etf.ultima_cota
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="text-[9px] font-mono text-[#999] uppercase tracking-widest">{sorted.length} ETFs</p>
